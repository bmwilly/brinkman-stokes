...
function set_coeff(self, coeff)
    return if (typeof(coeff) == ASCIIString)
...
function getUniformCoords(order, elems)
    return coords = linspace(0, 1, order * elems + 1)
end

function getElementCenters(order, elems)
    # order is ignored ...
    nodes = linspace(0, 1, elems + 1)
    return coords = 1 / 2 * (nodes[1:(end - 1)] + nodes[2:end])
end
