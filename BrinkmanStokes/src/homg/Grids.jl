...
struct Grid
	...
	Coarse::Any  # handle to coarse grid
		...
		if structof(mu) != Function && isinteger(mu)
			...
			if (grid.Mesh.dim == 2)
				...
				if (harmonic != 0) # if (harmonic)
					mu_coarse = 4 ./ (1.0 / mu2[1:2:end, 1:2:end] + 1.0 / mu2[2:2:end, 1:2:end] + 1.0 / mu2[1:2:end, 2:2:end] + 1.0 / mu2[2:2:end, 2:2:end])
				else
				...
				if (harmonic != 0) # if (harmonic)
					mu_coarse =
						8 ./ (
							1.0 / mu3[1:2:end, 1:2:end, 1:2:end] + 1.0 / mu3[2:2:end, 1:2:end, 1:2:end] +
							1.0 / mu3[1:2:end, 2:2:end, 1:2:end] + 1.0 / mu3[2:2:end, 2:2:end, 1:2:end] +
							1.0 / mu3[1:2:end, 1:2:end, 2:2:end] + 1.0 / mu3[2:2:end, 1:2:end, 2:2:end] +
							1.0 / mu3[1:2:end, 2:2:end, 2:2:end] + 1.0 / mu3[2:2:end, 2:2:end, 2:2:end]
						)
				else
					mu_coarse =
						0.125 * (
							mu3[1:2:end, 1:2:end, 1:2:end] + mu3[2:2:end, 1:2:end, 1:2:end] + mu3[1:2:end, 2:2:end, 1:2:end] + mu3[2:2:end, 2:2:end, 1:2:end] +
...
function get_eigenvectors(grid)
	...
	(evec, eval) = svd(Matrix(Kc))  # eig(Matrix(Kc), Matrix(grid.M));
