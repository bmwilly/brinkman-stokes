...
function gmres2(A::Function, b::Vector, restrt::Int; tol::Real = 1.0e-2, maxIter::Int = 100, M::Function = identity, x::Vector = [], out::Int = 0)
    ...
    for iter in 1:maxIter
        ...
        for i in 1:restrt
            ...
            for k in 1:i # basis using Gram-Schmidt
                H[k, i] = dot(w, V[:, k])
                w -= H[k, i] * V[:, k]
            end
            H[i + 1, i] = norm(w)
            ...
            for k in 1:(i - 1) # apply Givens rotation
